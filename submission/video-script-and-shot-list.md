...
reference, Bible version, and attribution. Open
...
frame **Send anyway** without clicking it.

**Narration:**

“Then Selah asks one private question. It never writes the reply, and it never
...
Helpline link. Add a compact audit overlay only after the stop is visible.

**Narration:**

“And if the message is more than angry, Selah does not answer a crisis with a
verse. It stops. No Scripture retrieval. No reflection. It points toward
...
“Selah doesn’t ask technology to speak for you. It gives you one quiet moment to
decide who you want to be—before your words leave your hands.”

## Recording and truth gates

- Do not record until a redacted production response reports `source: "live"`.
- Show an actual cursor and actual clicks; do not substitute static screenshots.
- Verify the safety probe stops before YouVersion retrieval and the reflection
  Gloo call. Use only the approved test phrase and redact request text from logs.
- Keep the public screen free of credentials, tokens, account details, and raw
  provider payloads.
- Burn readable captions, but keep them to two lines and below 12% of frame
  height. Never cover the product controls or provenance.
- Upload publicly to YouTube at 1080p, confirm no-login playback, then replace
  the Kaggle attachment and re-submit before the deadline.
